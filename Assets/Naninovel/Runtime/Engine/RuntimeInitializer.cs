...
namespace Naninovel
{
    ...
    public class RuntimeInitializer : MonoBehaviour
    {
        ...
        private static async UniTask PlayIntroMovie ()
        {
            ...
            await new PlayMovie { MovieName = cfg.IntroMovieName, Duration = 0 }.ExecuteAsync(Engine.DestroyToken);
        }

        private static async UniTask PlayInitScriptAsync ()
        {
            var scriptName = Engine.GetConfiguration<ScriptsConfiguration>().InitializationScript;
            ...
            {
                Engine.Warn($"Initialization script won't be played because '{player.PlayedScript.Name}' is currently playing. " +
            ...
            while (player.Playing)
                await AsyncUtils.WaitEndOfFrameAsync(Engine.DestroyToken);
        }
