...
namespace Naninovel
{
    ...
    public class TextManager : ITextManager
    {
        ...
        private readonly List<string> splitKeys = new List<string>();
        private readonly List<string> splitValues = new List<string>();
        private readonly List<string> splitComments = new List<string>();
            ...
            ILocalizationManager l10nManager, ICommunityLocalization communityL10n)
        {
            ...
            splitter = new TextSplitter(l10nManager.Configuration.RecordSeparator[0]);
        ...
        {
            return ParseDocument(resource.Object.text, GetCategory(resource), resource.Path);
        }

        protected virtual ManagedTextDocument ParseDocument (string text, string category, string name = null)
        {
            var doc = ManagedTextUtils.Parse(text, category, name);
            if (IsScriptCategory(category)) doc = SplitScriptRecords(doc);
            return doc;
        }
        ...
        protected virtual ManagedTextDocument SplitScriptRecords (ManagedTextDocument doc)
        {
            var records = new List<ManagedTextRecord>();
            var separator = l10nManager.Configuration.RecordSeparator[0];
            foreach (var record in doc.Records)
                if (!record.Key.Contains(separator)) records.Add(record);
                else records.AddRange(SplitScriptRecord(record));
            return new ManagedTextDocument(records, doc.Header);
        }

        protected virtual ManagedTextRecord[] SplitScriptRecord (ManagedTextRecord record)
        {
            splitKeys.Clear();
            splitValues.Clear();
            splitComments.Clear();
            ...
            for (int i = 0; i < split.Length; i++)
                split[i] = new ManagedTextRecord(splitKeys[i],
