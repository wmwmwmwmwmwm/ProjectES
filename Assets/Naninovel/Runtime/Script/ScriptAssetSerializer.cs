...
namespace Naninovel
{
    ...
    public class ScriptAssetSerializer
    {
        ...
        {
            this.stx = stx;
            scriptSerializer = new ScriptSerializer(stx);
            listSerializer = new ListValueSerializer(stx);
            namedSerializer = new NamedValueSerializer(stx);
        }
        ...
        private void AppendGenericLine (GenericTextScriptLine line)
        {
            ...
            void DelimitStartWhiteSpace ()
            {
                if (line.InlinedCommands.Skip(shouldSkipFirst ? 1 : 0).FirstOrDefault() is PrintText text &&
                    ...
                    char.IsWhiteSpace(SerializeRaw(text.Text.RawValue.Value, new SerializationContext()).FirstOrDefault()))
            ...
            void DelimitEndWhiteSpace ()
            {
                if (line.InlinedCommands.LastOrDefault() is PrintText text &&
                    text.Text?.RawValue != null &&
                    char.IsWhiteSpace(SerializeRaw(text.Text.RawValue.Value, new SerializationContext()).LastOrDefault()))
                    builder.Append(stx.InlinedOpen).Append(stx.InlinedClose);
            }
        ...
        private string SerializeRaw (RawValue raw, SerializationContext context)
        {
            ...
            foreach (var part in raw.Parts)
                if (part.Kind == ParameterValuePartKind.IdentifiedText)
                    ...
                    components.Add(new Parsing.Expression(new PlainText(part.Expression)));
