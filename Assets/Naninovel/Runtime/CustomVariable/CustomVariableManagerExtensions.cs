...
namespace Naninovel
{
    ...
    public static class CustomVariableManagerExtensions
    {
        ...
        public static bool TrySetVariableValue<TValue> (this ICustomVariableManager manager, string name, TValue value)
        {
            if (typeof(TValue) == typeof(string))
            {
                manager.SetVariableValue(name, new CustomVariableValue(Convert.ToString(value)));
                return true;
            }

            if (typeof(TValue) == typeof(float) || typeof(TValue) == typeof(double) || typeof(TValue) == typeof(int))
            {
                manager.SetVariableValue(name, new CustomVariableValue(Convert.ToSingle(value)));
                return true;
            }

            if (typeof(TValue) == typeof(bool))
            {
                manager.SetVariableValue(name, new CustomVariableValue(Convert.ToBoolean(value)));
                return true;
            }
