...
namespace Naninovel
{
    ...
    public class CustomVariableManager : IStatefulService<GameStateMap>, IStatefulService<GlobalStateMap>, ICustomVariableManager
    {
        ...
        private readonly Dictionary<string, CustomVariable> variablesByName = new Dictionary<string, CustomVariable>();
        ...
        {
            return variablesByName.Values.ToArray();
        }
        ...
        public virtual void SetVariableValue (string name, CustomVariableValue value)
        {
            ...
            {
                variablesByName[name] = new CustomVariable(name, scope, value);
                OnVariableUpdated?.Invoke(new CustomVariableUpdatedArgs(name, value, null));
                return;
            }
        ...
        public virtual void ResetVariable (string name)
        {
            ...
            else OnVariableUpdated?.Invoke(new CustomVariableUpdatedArgs(name, null, var.Value));
        }

        public virtual void ResetAllVariables (CustomVariableScope? scope = null)
        {
            foreach (var var in GetAllVariables())
                if (scope == null || var.Scope == scope.Value)
        ...
        protected virtual void ResetPredefinedVariable (string name, string predefine)
        {
            ...
            if (operand is Expression.String str) SetVariableValue(name, new CustomVariableValue(str.Value));
            else if (operand is Expression.Boolean boo) SetVariableValue(name, new CustomVariableValue(boo.Value));
            else SetVariableValue(name, new CustomVariableValue((float)(double)operand.GetValue()));
        }
    }
}
