...
namespace Naninovel
{
    ...
    public class CustomVariableTrigger : MonoBehaviour
    {
        ...
        [Tooltip("Invoked when value of a custom variable with specified name is changed and the value is string.")]
        [SerializeField] private StringChangedEvent onStringChanged;
        [Tooltip("Invoked when value of a custom variable with specified name is changed and the value is a number.")]
        ...
        protected virtual void NotifyValueChanged (CustomVariableValue? value)
        {
            ...
            var v = value.Value;
            if (v.Type == CustomVariableValueType.String) onStringChanged?.Invoke(v.String);
            else if (v.Type == CustomVariableValueType.Numeric) onNumericChanged?.Invoke(v.Number);
            else onBooleanChanged?.Invoke(v.Boolean);
        }
