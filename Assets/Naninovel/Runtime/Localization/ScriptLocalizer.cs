...
namespace Naninovel
{
    ...
    public class ScriptLocalizer
    {
        ...
        public class Options
        {
            ...
            /// <summary>
            /// Character to join composite parts of the localized content, such as parts of generic text lines.
            /// </summary>
        ...
        public ScriptLocalizer (Options options = null)
        {
            ...
            splitter = new TextSplitter(this.options.Separator);
            serde = new ScriptAssetSerializer(this.options.Syntax);
        }
        ...
        public ManagedTextDocument Localize (Script script, ManagedTextDocument existing = null)
        {
            ...
            return new ManagedTextDocument(records.ToArray());
        }
        ...
        {
            if (!(command is Command.ILocalizable)) return;
        ...
        private void CollectLocalizableContent (string mapId)
        {
            ...
            if (text is null) throw new Error($"Failed to extract script text with '{mapId}' ID. Make sure '{script.Name}' script asset is imported without errors.");
            this.text.Add(text);
        }
        ...
        private void AppendRecord (string annotation = null)
        {
            ...
            records.Add(new ManagedTextRecord(key, value, comment));
            ids.Clear();
            text.Clear();
        }
        ...
        {
            if (!(line is GenericTextScriptLine generic)) return false;
            foreach (var inlined in generic.InlinedCommands)
                if (!(inlined is PrintText print)) return false;
