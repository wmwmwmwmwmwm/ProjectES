...
namespace Naninovel
{
    ...
    public class LocalizationEjector
    {
        ...
        {
            var outPath = Path.Combine(outDir, $"{script.Name}.txt");
            var category = $"{ManagedTextConfiguration.ScriptMapCategory}/{script.Name}";
            var existingDoc = File.Exists(outPath) ? ManagedTextUtils.Parse(File.ReadAllText(outPath), category) : null;
            ...
            var output = ManagedTextUtils.Serialize(document, category);
            File.WriteAllText(outPath, output);
        }

        private void EjectText (string sourceText, string category, string outDir)
        {
            var outPath = Path.Combine(outDir, $"{category}.txt");
            var sourceDoc = ManagedTextUtils.Parse(sourceText, category);
            var existingDoc = File.Exists(outPath) ? ManagedTextUtils.Parse(File.ReadAllText(outPath), category) : null;
