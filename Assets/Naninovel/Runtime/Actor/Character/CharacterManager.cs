...
namespace Naninovel
{
    ...
    public class CharacterManager : OrthoActorManager<ICharacterActor, CharacterState, CharacterMetadata, CharactersConfiguration>, ICharacterManager
    {
        ...
        public CharacterManager (CharactersConfiguration config, CameraConfiguration cameraConfig,
            ITextManager textManager, ITextPrinterManager textPrinterManager)
        ...
        public virtual string GetAuthorName (string characterId)
        {
            ...
            var displayName = textManager.GetRecordValue(characterId, CharactersConfiguration.DisplayNamesCategory);
        ...
        protected virtual void HandleAuthorHighlighting (PrintTextArgs args)
        {
            ...
            var authorIds = (args.Author.Id ?? "").Split(',');
            ...
            foreach (var actor in ManagedActors.Values)
            {
                ...
                var speaking = actorMeta.HighlightCharacterCount > visibleActors || authorIds.Contains(actor.Id) || args.Author.Id == "*";
            ...
            if (string.IsNullOrEmpty(args.Author.Id) || !ActorExists(args.Author.Id)) return;
            var authorMeta = Configuration.GetMetadataOrDefault(args.Author.Id);
            if (authorMeta.HighlightWhenSpeaking && authorMeta.HighlightCharacterCount <= visibleActors && authorMeta.PlaceOnTop)
            {
                ...
                if (topmostChar != null && !topmostChar.Id.EqualsFast(args.Author.Id))
                {
                    var authorChar = GetActor(args.Author.Id);
