...
namespace Naninovel
{
        ...
        where TMeta : OrthoActorMetadata
    {
            ...
            Transition? transition = default, AsyncToken asyncToken = default)
        {
            ...
            videoAppearance.Video.aspectRatio = FindString(appearance, "FitHor") ? VideoAspectRatio.FitHorizontally : VideoAspectRatio.FitInside;
            videoAppearance.Video.targetTexture = RenderTexture.GetTemporary(Screen.width, Screen.height, TextureDepthBuffer, TextureFormat);//(int)video.width, (int)video.height, TextureDepthBuffer, TextureFormat);
            videoAppearance.Video.time = 0;
        ...
        protected virtual async UniTask<VideoAppearance> GetAppearanceAsync (string videoName, AsyncToken asyncToken = default)
        {
            ...
            var videoPlayer = Engine.CreateObject<VideoPlayer>(videoName, GameObject.layer, Transform);
            videoPlayer.playOnAwake = false;
            videoPlayer.isLooping = !FindString(videoName, "NoLoop");
            ...
            var videoAppearance = new VideoAppearance(videoPlayer, SetupAudioSource(videoPlayer, videoName));
            Appearances[videoName] = videoAppearance;

            return videoAppearance;
        }

        protected virtual bool FindString(string appearance, string check)
        {
            return appearance.IndexOf(check, StringComparison.OrdinalIgnoreCase) >= 0;
        }

        protected virtual AudioSource SetupAudioSource (VideoPlayer player, string videoName)
        {
            ...
            else
            {
                ...
                {
                    if (FindString(videoName, "Sfx"))
                        audioSource.outputAudioMixerGroup = audioManager.AudioMixer.FindMatchingGroups(audioManager.Configuration.SfxGroupPath)?.FirstOrDefault();
                    else if (FindString(videoName, "Voice"))
                        audioSource.outputAudioMixerGroup = audioManager.AudioMixer.FindMatchingGroups(audioManager.Configuration.VoiceGroupPath)?.FirstOrDefault();
                    else if (FindString(videoName, "Bgm"))
                        audioSource.outputAudioMixerGroup = audioManager.AudioMixer.FindMatchingGroups(audioManager.Configuration.BgmGroupPath)?.FirstOrDefault();
                    else
                        audioSource.outputAudioMixerGroup = audioManager.AudioMixer.FindMatchingGroups(audioManager.Configuration.MasterGroupPath)?.FirstOrDefault();
                }
