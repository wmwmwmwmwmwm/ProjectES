...
namespace Naninovel
{
    ...
    public class TextPrinterManager : OrthoActorManager<ITextPrinterActor, TextPrinterState, TextPrinterMetadata, TextPrintersConfiguration>, IStatefulService<SettingsStateMap>, ITextPrinterManager
    {
        ...
        public virtual async UniTask PrintTextAsync (string printerId, LocalizableText text,
            AuthorInfo author = default, float speed = 1, AsyncToken asyncToken = default)
        {
            var printer = await GetOrAddActorAsync(printerId);
            var meta = Configuration.GetMetadataOrDefault(printerId);
            asyncToken.ThrowIfCanceled();

            OnPrintTextStarted?.Invoke(new PrintTextArgs(printer, text, author, speed));

            printer.Author = author;
            printer.Text += text;

            var delay = (meta.RevealInstantly || scriptPlayer.SkipActive) ? 0 : Mathf.Lerp(Configuration.MaxRevealDelay, 0, BaseRevealSpeed * speed);
            await printer.RevealTextAsync(delay, asyncToken);

            OnPrintTextFinished?.Invoke(new PrintTextArgs(printer, text, author, speed));
        }
    }
}
