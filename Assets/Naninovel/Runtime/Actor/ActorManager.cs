...
namespace Naninovel
{
        ...
        where TConfig : ActorManagerConfiguration<TMeta>
    {
        ...
        protected ActorManager (TConfig config)
        {
            ...
            AppearanceLoaders = new Dictionary<string, IResourceLoader>(StringComparer.Ordinal);
            pendingAddActorTasks = new Dictionary<string, UniTaskCompletionSource<TActor>>();
        }

        public virtual UniTask InitializeServiceAsync ()
        {
            if (implNameToType is null)
                implNameToType = Engine.Types
        ...
        public IResourceLoader GetAppearanceLoader (string actorId)
        {
            ...
            loader = (IResourceLoader)Activator.CreateInstance(loaderType, actorId, meta,
                Engine.GetService<IResourceProviderManager>(), Engine.GetService<ILocalizationManager>());
