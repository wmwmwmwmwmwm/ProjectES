...
namespace Naninovel
{
    ...
    public class UIManager : IUIManager, IStatefulService<SettingsStateMap>
    {
        ...
        public virtual void SetUIVisibleWithToggle (bool visible, bool allowToggle = true)
        {
            ...
            //    .Where(u => u.HideInThumbnail && u.TopmostCanvas.enabled != visible)
            ...
            var disabledCanvases = Engine.GetService<IUIManager>()
                .GetManagedUIs().OfType<CustomUI>()
                .Where(u => !(u is ClickThroughPanel) && u.TopmostCanvas.enabled != visible)
                .Select(u => u.TopmostCanvas).ToArray();
        ...
        protected virtual IManagedUI InstantiatePrefab (GameObject prefab, string name = default, string group = default)
        {
            ...
            if (canvas)
            {
                canvas.planeDistance = Configuration.planeDistance;
                canvas.renderMode = Configuration.renderMode;
            }
        ...
        protected virtual async UniTask<IEnumerable<(string Name, TMP_FontAsset Asset)>> InitializeFontsAsync ()
        {
            if (communityL10n.Active)
            {
                var font = await communityL10n.LoadFontAsync();
