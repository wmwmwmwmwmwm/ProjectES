    ...
    public class DebugInfoGUI : MonoBehaviour
    {
        ...
        private void HandleActionExecuted (Command command)
        {
            ...
            if (audioManager != null && audioManager.Configuration.EnableAutoVoicing && command is PrintText print)
                lastAutoVoiceName = audioManager.Configuration.AutoVoiceMode == AutoVoiceMode.TextId
                    ? AudioConfiguration.GetAutoVoiceClipPath(print.Text)
                    : AudioConfiguration.GetAutoVoiceClipPath(print.PlaybackSpot);
        }
