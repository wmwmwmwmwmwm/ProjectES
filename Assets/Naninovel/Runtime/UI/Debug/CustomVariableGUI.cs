    ...
    public class CustomVariableGUI : MonoBehaviour
    {
        ...
        private void SetEditedValue (string name, string editedValue)
        {
            if (editedValue.StartsWithFast("\"") && editedValue.EndsWithFast("\""))
                variableManager.SetVariableValue(name, new CustomVariableValue(editedValue.Substring(1, editedValue.Length - 2)));
            else if (bool.TryParse(editedValue, out var boo))
                variableManager.SetVariableValue(name, new CustomVariableValue(boo));
            else if (ParseUtils.TryInvariantFloat(editedValue, out var num))
                variableManager.SetVariableValue(name, new CustomVariableValue(num));
        }
        ...
        private void HandleVariableUpdated (CustomVariableUpdatedArgs args)
        {
            ...
            records.Add(args.Name, new Record(args.Name, value));
        }
        ...
        private void UpdateRecords ()
        {
            ...
            foreach (var variable in variableManager.GetAllVariables())
                records.Add(variable.Name, new Record(variable.Name, ToRecordValue(variable.Value)));
        }
    }
}
