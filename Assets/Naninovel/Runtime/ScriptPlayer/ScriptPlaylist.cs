...
namespace Naninovel
{
    ...
    public class ScriptPlaylist : IReadOnlyList<Command>
    {
        ...
        /// </summary>
        public async UniTask LoadResourcesAsync () => await LoadResourcesAsync(0, Count - 1);

        /// <summary>
        /// Preloads and holds resources required to execute
        /// <see cref="Command.IPreloadable"/> commands in the specified range.
        /// </summary>
        public async UniTask LoadResourcesAsync (int startCommandIndex, int endCommandIndex, Action<float> onProgress = default)
        {
            if (Count == 0) return;
