...
namespace Naninovel
{
    ...
    public static class ExpressionEvaluator
    {
        ...
        private static readonly List<ParseDiagnostic> errors = new List<ParseDiagnostic>();
        private static readonly List<ExpressionFunction> functions = new List<ExpressionFunction>();
        private static Parser parser;
        private static Evaluator evaluator;

        public static void Initialize ()
        {
            parser = new Parser(new Expression.ParseOptions {
                Syntax = Compiler.Syntax,
                HandleDiagnostic = errors.Add
            });
            evaluator = new Evaluator(new EvaluateOptions {
                ResolveVariable = ResolveVariable,
                ResolveFunction = ResolveFunction
            });
            foreach (var fn in ExpressionFunctions.Resolve())
                functions.Add(fn);
        }
        ...
        private static void EnsureInitialized ()
        {
            ...
            Debug.Assert(parser != null && evaluator != null && functions != null);
        }

        private static IOperand ResolveVariable (string name)
        {
            if (name.StartsWith(Compiler.TextVariablePrefix, StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (string.IsNullOrEmpty(managedTextValue))
                    Engine.Warn($"Missing '{name}' managed text variable. Make sure associated record exists in '{ManagedTextScriptCategory}' managed text document.");
            ...
            {
                vars.TrySetVariableValue(name, false);
        ...
        private static object InvokeMethod (string name, object[] args, bool strictType)
        {
            ...
            {
                if (!fn.Id.EqualsFastIgnoreCase(name)) continue;
            ...
            {
                if (strictType) return actual.GetType() == expected;
