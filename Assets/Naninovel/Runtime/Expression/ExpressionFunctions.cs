...
namespace Naninovel
{
    ...
    public static class ExpressionFunctions
    {
        ...
        [Documentation("Return a random integer number between min [inclusive] and max [inclusive].", examples: "random(0, 100)")]
        public static int Random (int min, int max) => UnityEngine.Random.Range(min, max + 1);

        [ExpressionFunction("random")]
        [Documentation("Return a string chosen from one of the provided strings.", examples: "random(\"foo\", \"bar\", \"baz\")")]
        public static string Random (params string[] args) => args.Random();

        [ExpressionFunction("calculateProgress")]
        [Documentation("Returns scenario completion ratio, in 0.0 to 1.0 range, where 1.0 means all the script lines were executed at least once.", examples: "calculateProgress()")]
        public static float CalculateProgress ()
        {
            ...
            {
                Engine.Warn("'calculateProgress' expression function was used, while to total number of script commands is zero. You've most likely disabled 'Update Action Count On Init' in the script player configuration menu or didn't add any naninovel scripts to the project resources.");
                return 0;
            }
        ...
        [Documentation("Checks whether an unlockable item with the provided ID is currently unlocked.", examples: "isUnlocked(\"Tips/MyTip\")")]
        ...
        [Documentation("Checks whether currently played command has ever been played before.", examples: "hasPlayed()")]
        ...
        [ExpressionFunction("hasPlayed")]
        [Documentation("Checks whether script with the provided name has ever been played before.", examples: "hasPlayed(\"MyScript\")")]
        public static bool HasPlayed ([ResourceContext(ScriptsConfiguration.DefaultPathPrefix)] string script)
        {
            return Engine.GetService<IScriptPlayer>().HasPlayed(script);
        }

        [ExpressionFunction("getName")]
        [Documentation("Returns author name of a character actor with the provided ID.", examples: "getName(\"Kohaku\")")]
        public static string GetName ([ActorContext(CharactersConfiguration.DefaultPathPrefix)] string id)
        {
            return Engine.GetService<ICharacterManager>().GetAuthorName(id);
        }

        [ExpressionFunction("pow")]
        [Documentation("Returns num raised to power.", examples: "pow(2, 3)")]
        public static double Pow (double num, double pow) => Mathf.Pow((float)num, (float)pow);

        [ExpressionFunction("sqrt")]
        [Documentation("Returns square root of num.", examples: "sqrt(2)")]
        public static double Sqrt (double num) => Mathf.Sqrt((float)num);

        [ExpressionFunction("cos")]
        [Documentation("Returns the cosine of angle.", examples: "cos(180)")]
        public static double Cos (double num) => Mathf.Cos((float)num);

        [ExpressionFunction("sin")]
        [Documentation("Returns the sine of angle.", examples: "sin(90)")]
        public static double Sin (double num) => Mathf.Sin((float)num);

        [ExpressionFunction("log")]
        [Documentation("Returns the natural (base e) logarithm of a specified number.", examples: "log(0.5)")]
        public static double Log (double num) => Mathf.Log((float)num);

        [ExpressionFunction("abs")]
        [Documentation("Returns the absolute value of f.", examples: "abs(0.5)")]
        public static double Abs (double num) => Mathf.Abs((float)num);

        [ExpressionFunction("max")]
        [Documentation("Returns largest of two or more values.", examples: "max(1, 10, -9)")]
        public static double Max (params double[] nums) => Mathf.Max(nums.Select(n => (float)n).ToArray());

        [ExpressionFunction("min")]
        [Documentation("Returns the smallest of two or more values.", examples: "min(1, 10, -9)")]
        public static double Min (params double[] nums) => Mathf.Min(nums.Select(n => (float)n).ToArray());

        [ExpressionFunction("round")]
        [Documentation("Returns num rounded to the nearest integer.", examples: "round(0.9)")]
        public static double Round (double num) => Mathf.Round((float)num);

        [ExpressionFunction("approx")]
        [Documentation("Compares two floating point values and returns true if they are similar.", examples: "approx(0.15, 0.15)")]
            ...
            foreach (var type in Engine.Types)
                if (type.IsPublic && type.IsClass && type.IsAbstract && type.IsSealed)
                    foreach (var method in type.GetMethods(BindingFlags.Static | BindingFlags.Public))
                        if (method.GetCustomAttribute<ExpressionFunctionAttribute>() is ExpressionFunctionAttribute fn)
        ...
        private static ExpressionFunction BuildFunction (MethodInfo method, ExpressionFunctionAttribute fn)
        {
            ...
            return new ExpressionFunction(id, method, doc?.Summary, doc?.Remarks, doc?.Examples);
        }
    }
}
