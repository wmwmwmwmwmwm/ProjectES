...
namespace Naninovel
{
    public static class ParseUtils
    {
        ...
        /// <summary>
        /// Attempts to parse an enum value assigned to script command parameter with contant context.
        /// Will consider contants localized via <see cref="Compiler"/>.
        /// </summary>
        /// <returns>Whether parsing succeeded.</returns>
        public static bool TryConstantParameter<TEnum> (string value, out TEnum result) where TEnum : struct
        {
            ...
            if (Compiler.Constants.TryGetValue(typeof(TEnum).Name, out var l10n))
                if (l10n.Values.FirstOrDefault(v => v.Alias.EqualsFastIgnoreCase(value)) is ConstantValueLocalization vn)
                    value = vn.Value;
