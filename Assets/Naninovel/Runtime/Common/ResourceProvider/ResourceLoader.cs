        ...
        where TResource : UnityEngine.Object
    {
        ...
        public virtual string GetLocalPath (string fullPath)
        {
            foreach (var res in LoadedResources)
                if (res.FullPath.EqualsFast(fullPath))
        ...
        {
            var resource = GetLoadedResourceOrNull(path);
            if (resource != null) Release(resource, holder, unload);
        }

        public virtual void ReleaseAll (object holder, bool unload = true)
        {
            foreach (var res in LoadedResources.ToArray())
        ...
        public virtual int CountHolders (string path)
        {
            return GetLoadedResourceOrNull(path)?.HoldersCount ?? 0;
        }

        public virtual bool IsLoaded (string path)
        {
            foreach (var res in LoadedResources)
                if (res.Valid && res.LocalPath.EqualsFast(path))
                    return true;
            return false;
        }
        ...
        {
            if (LoadedResources.Count == 0) return Array.Empty<Resource<TResource>>();
            var result = new List<Resource<TResource>>();
            foreach (var res in LoadedResources)
                if (res.Valid)
        ...
        {
            foreach (var res in LoadedResources)
                if (res.Valid && res.LocalPath.EqualsFast(localPath))
                    return res;
            return null;
        }
        ...
        protected virtual void Release (LoadedResource resource, object holder, bool unload = true)
        {
            ...
            if (resource.Valid && (resource.HoldersCount > 0 || HoldersTracker.Release(resource.Object, this) > 0 || !unload)) return;
