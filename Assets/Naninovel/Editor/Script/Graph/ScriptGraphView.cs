    ...
    public class ScriptGraphView : GraphView
    {
        ...
        public void SerializeState ()
        {
            ...
            foreach (var node in scriptNodes)
                if (!string.IsNullOrEmpty(AssetDatabase.GUIDToAssetPath(node.ScriptGuid)))
                    state.NodesState.Add(new ScriptGraphNodeState(node.ScriptName, node.GetPosition()));
                else Debug.Log($"FAAAAIl {node.ScriptName}");
        ...
        private void RebuildGraph ()
        {
            ...
            for (int i = 0; i < scriptNodes.Count; i++)
            {
                ...
                {
                    var gotoNode = scriptNodes.FirstOrDefault(n => n.ScriptName == data.ScriptName);
