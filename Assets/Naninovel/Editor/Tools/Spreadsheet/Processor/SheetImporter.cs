...
namespace Naninovel.Spreadsheet
{
    public class SheetImporter
    {
        ...
        protected virtual Dictionary<int, string> IndexToKey { get; } = new Dictionary<int, string>();
        ...
        protected virtual void ImportLocaleColumn (SheetColumn column)
        {
            ...
            for (int i = 1; i < column.Cells.Count; i++)
            {
                ...
                if (index >= 0) records[index] = new ManagedTextRecord(key, column.Cells[i], records[index].Comment);
        ...
        protected virtual bool TryLoadL10nDocument (string locale, out ManagedTextDocument doc, out string path)
        {
            ...
            doc = File.Exists(path) ? ManagedTextUtils.Parse(File.ReadAllText(path), Category, path) : null;
